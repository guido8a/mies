...
class ModificacionController extends mies.seguridad.Shield {
...
//        def asignaciones = Asignacion.findAllByMarcoLogicoAndAnio(asg.marcoLogico,asg.anio)
        ...
        if (asg.errors.getErrorCount() == 0) {
            ...
            mod.desde = asg
            mod.valorDesde=asg.planificado
            mod.recibe = null
    ...
    def guardarReasignacion = {
        ...
        if (f && !f.empty) {
            ...
            fileName = ""
            //println "filename "+fileName+" e"+parts
            ...
            fileName = fileName + "_" + new Date().format("hh_mm_ss") + "." +((ext)?ext:"pdf")
            //println "file name 2 "+fileName
            ...
            if (src.exists()) {
                ...
                if (params.destino != "") {
                    ...
                    mod.fecha = new Date()
                    mod.valorDesde=origen.planificado
                    mod.valorHasta=destino.planificado
                    ...
                    if (origen.errors.getErrorCount()!=0){
                        band=false
                        println "SVT-ERROR guardar asgn mod "+origen.id+"   "+origen.errors
                    }
                    ...
                    if (destino.errors.getErrorCount()!=0){
                        println "SVT-ERROR guardar asgn mod recibe "+destino.id+"   "+destino.errors
                        band=false
                        origen.planificado = origen.planificado + params.monto.toDouble()
                        destino.planificado = destino.planificado - params.monto.toDouble()
                        origen = kerberosService.saveObject(origen, Asignacion, session.perfil, session.usuario, "guardarReasignacion_error", "modificacion", session)
                    }
                    //guardarPras(origen)
                    //guardarPras(destino)
                    if(band){
                        ...
                        msn = "Error al procesar la modificacíon"
                        if (params.tipoPag)
                            redirect(action: 'poaInversionesMod', params: [msn: msn, id: params.unidad])
                        else
                            redirect(action: 'poaCorrientesMod', params: [msn: msn, id: params.unidad])
