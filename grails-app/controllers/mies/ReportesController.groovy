...
//        resto+= UnidadEjecutora.findAllByCodigoIsNull()
    ...
    def reporteIntervencion(params) {
        ...
        if (tipo == "N") {
            ...
            tabla += '</th>'
            tabla += '<th>'
            tabla += 'PROGRAMA O INSTITUTO DEL MIES'
            tabla += '</th>'
            tabla += '</tr>'
            tabla += '</thead>'
...
//                    def ejecuciones = Ejecucion.findAllByAsignacion(asg)
    ...
    def reportePac(ids, anioId) {
        def anio = Anio.get(anioId)
        def mies = UnidadEjecutora.findByCodigo("280")
...
//        }
        ...
        sheet.addCell(label);
        label = new Label(4, 1, "Política MIES", times16format);
        sheet.addCell(label);
...
//                            sheet.addCell(number);
                ...
                if(total.toFloat().round(2)!=inv.planificado.toFloat().round(2)){
                    ...
                    sheet.addCell(label);
                    label = new Label(6, columna, inv.marcoLogico.proyecto.programaPresupuestario.descripcion);
                    sheet.addCell(label);
...
//        }
        ...
        sheet.addCell(label);
        label = new Label(5, 1, "Política MIES", times16format);
        sheet.addCell(label);
                    ...
                    programaMap.proyectos.each { proyectoMap ->
                        proyectoMap.actividades.each { actividadMap ->
                            actividadMap.asignaciones.each { asignacion ->
                                ...
                                inversion += format("MIES") + sep
                                label = new Label(0, fila, "MIES");
                                sheet.addCell(label);
