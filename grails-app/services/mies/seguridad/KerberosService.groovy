...
//                println anterior?.class.toString()
                if (anterior && anterior.class.toString() =~ "mies") {
        ...
        try {
            ...
            if (nuevo.id == null) { //Insert
                ...
                for (name in names) {
                    ...
                    if (originalValue?.class?.toString() =~ "mies") {
    ...
    public saveModificacion(nuevo, dominio,usuario, session) {
        ...
        try {
            ...
            if (nuevo.id == null) { //Insert
                ...
                def mod = new mies.ModificacionV2()

                if (nuevo.save(flush: false)) {

                    mies.ModificacionV2.withTransaction { status ->
                ...
                for (name in names) {
                    ...
                    if (!ignore.contains(name)) {
                        ...
                        def mod = new mies.ModificacionV2([usuario:usuario,campo:name,dominio:new org.codehaus.groovy.grails.commons.DefaultGrailsDomainClass(dominio).getName(),id_remoto:old.id.toInteger(),newValue:newValue,oldValue:originalValue,tipo:tipo])
                        listAudt.add(mod)
                        mod =null
